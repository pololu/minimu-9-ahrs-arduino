...
Open the sketch in the Arduino environment by opening one of the .pde files in the MinIMU9ArduinoAHRS folder.

By default, the Arduino code treats the positive X axis of the MinIMU-9 as forward and the negative Z axis as up (so that the components are on the bottom of the board and the silkscreen is on top). If you want the positive Z axis to be up instead (so that the components are on top), find the definition of @SENSOR_SIGN[9]@ at the beginning of MinIMU9ArduinoAHRS.pde and uncomment the appropriate line (after commenting the original). Then, upload the program to your Arduino.
